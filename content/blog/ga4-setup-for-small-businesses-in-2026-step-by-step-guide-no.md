...
- **Google Tag Manager installed** (optional but strongly recommended, as it makes future changes 10x faster)
...
**1. Destination-based conversions** (easiest): Fire when a user reaches a specific URL, such as your confirmation page, a booking confirmation, or a checkout success page. This works for most small businesses.
...
**Mistake 2: Not setting the correct data retention period.** GA4 defaults to 2 months of event data retention. Change it to 14 months under Admin > Data Settings > Data Retention. Do this on day one, as you can't retroactively recover data.
...
For most small businesses, the DIY approach covers nearly all the value. The only scenario where hiring makes sense is if you need custom event tracking tied to a complex checkout flow or a CRM integration (like firing GA4 events when a HubSpot deal closes).
...
Yes, Google Analytics 4 is completely free for the standard version, which covers everything in this guide including custom events, conversions, Explorations, and Search Console integration. GA4 360 (the enterprise paid version) [starts at roughly $50,000/year](https://www.cardinalpath.com/blog/ua-360-vs-ga4-360-pricing-model) and is not relevant for small businesses.
